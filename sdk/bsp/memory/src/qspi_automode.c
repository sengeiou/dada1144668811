 ...
 */
QSPI_SECTION static inline void flash_reset_continuous_mode(HW_QSPI_BREAK_SEQ_SIZE break_seq_size) __attribute__((always_inline));
 ...
 *
 */
QSPI_SECTION static bool qspi_writable(void)
{
        bool writable;
 ...
 */
QSPI_SECTION static size_t write_page(uint32_t addr, const uint8_t *buf, uint16_t size)
{
        size_t written;
 ...
 * \param[in] addr The address of the sector to be erased.
 */
QSPI_SECTION static void erase_sector(uint32_t addr)
{
