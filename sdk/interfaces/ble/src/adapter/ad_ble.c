...
#warning "BLE Adapter: GPIO debugging is on!"
#endif

#if (dg_configNVMS_ADAPTER == 0)
#warning "NVMS Adapter is disabled. BLE device is going to use default BD_ADDR and IRK."
#endif
 ...
 */
static void ad_ble_send_to_stack(const irb_ble_stack_msg_t *ptr_msg);
 ...
 */
static void ad_ble_handle_stack_msg(irb_ble_stack_msg_t *msg);
                        ...
                        if ( OS_QUEUE_GET(adapter_if.cmd_q, &received_msg, 0)) {
                                ...
                                if(current_op == AD_BLE_OP_CODE_STACK_MSG) {
                                        ...
                                        irb_ble_stack_msg_t *stack_msg =
                                                               (irb_ble_stack_msg_t *) received_msg;
#ifdef BLE_PROD_TEST
...
void ad_ble_init(void)
{
        ...
        OS_QUEUE_CREATE(adapter_if.cmd_q, sizeof(irb_ble_stack_msg_t *), AD_BLE_COMMAND_QUEUE_LENGTH);
        OS_QUEUE_CREATE(adapter_if.evt_q, sizeof(irb_ble_stack_msg_t *), AD_BLE_EVENT_QUEUE_LENGTH);
...
void ad_ble_send_to_stack(const irb_ble_stack_msg_t *ptr_msg)
{
        ble_stack_msg_type_t msg_type = ptr_msg->msg_type;
        volatile uint16_t msgSize = ptr_msg->msg_size + sizeof(uint8_t);
        volatile uint8_t *msgPtr = (uint8_t *) &ptr_msg->msg;
#if (dg_configBLE_ADV_STOP_DELAY_ENABLE == 1)
        ...
        if (ble_stack_io_rx_buf_ptr != NULL) {
                ...
                *ble_stack_io_rx_buf_ptr++ = msg_type;
                ...
                while (msgSize) {
                        *ble_stack_io_rx_buf_ptr++ = *msgPtr++;
                        msgSize--;
                        ble_stack_io_rx_size_req--;

                        // Check if all expected data from the BLE stack have been read
                        if (ble_stack_io_rx_size_req == 0) {
                                // Call the BLE stack to decide whether more data are needed
                                ble_stack_io_rx_done_cb(BLE_STACK_IO_OK);
                        }
                }
        }
...
void ble_stack_io_write(uint8_t* bufPtr, uint32_t size, void (*callback) (uint8_t))
{
...
#endif
        {
                ...
                msgBuf = OS_MALLOC(sizeof( irb_ble_stack_msg_t ) + param_length - sizeof(uint32_t));

                msgBuf->op_code = IRB_BLE_STACK_MSG;                // fill IRB message OP code
                msgBuf->msg_type = *pxMsgPacked++;                  // fill stack message type
                msgBuf->msg_size = header_length + param_length;    // fill total message size

                // copy the rest of the message
                memcpy(&msgBuf->msg, pxMsgPacked, msgBuf->msg_size);
