...
inv_error_t set_output_rates(float rate)
{
        ...
        if (hal.report & PRINT_ACCEL)
                result |= inv_set_odr(ANDROID_SENSOR_ACCELEROMETER,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_GYRO)
                result |= inv_set_odr(ANDROID_SENSOR_GYROSCOPE, (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_RAW_GYRO)
                result |= inv_set_odr(ANDROID_SENSOR_GYROSCOPE_UNCALIBRATED,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_GRV)
                result |= inv_set_odr(ANDROID_SENSOR_GAME_ROTATION_VECTOR,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_CUBE_GRV)
                result |= inv_set_odr(ANDROID_SENSOR_GAME_ROTATION_VECTOR,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_STEP_DETECTOR)
                result |= inv_set_odr(ANDROID_SENSOR_STEP_DETECTOR,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_SMD)
                result |= inv_set_odr(ANDROID_SENSOR_WAKEUP_SIGNIFICANT_MOTION,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_STEP_COUNTER)
                result |= inv_set_odr(ANDROID_SENSOR_STEP_COUNTER,
                        (unsigned short)(base_rate / rate));
#if (MEMS_CHIP == HW_ICM20648)
        if (hal.report & PRINT_LINEAR_ACCEL)
                result |= inv_set_odr(ANDROID_SENSOR_LINEAR_ACCELERATION,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_GRAVITY)
                result |= inv_set_odr(ANDROID_SENSOR_GRAVITY, (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_COMPASS)
                result |= inv_set_odr(ANDROID_SENSOR_GEOMAGNETIC_FIELD,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_RAW_COMPASS)
                result |= inv_set_odr(ANDROID_SENSOR_MAGNETIC_FIELD_UNCALIBRATED,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_RV)
                result |= inv_set_odr(ANDROID_SENSOR_ROTATION_VECTOR,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_GEOMAG)
                result |= inv_set_odr(ANDROID_SENSOR_GEOMAGNETIC_ROTATION_VECTOR,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_BAC)
                result |= inv_set_odr(ANDROID_SENSOR_ACTIVITY_CLASSIFICATON,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_TILT)
                result |= inv_set_odr(ANDROID_SENSOR_WAKEUP_TILT_DETECTOR,
                        (unsigned short)(base_rate / rate));
        if (hal.report & PRINT_FLIP_PICKUP)
                result |= inv_set_odr(ANDROID_SENSOR_FLIP_PICKUP,
                        (unsigned short)(base_rate / rate));
        ...
        if (hal.report & PRINT_ORIENT)
                result |= inv_set_odr(ANDROID_SENSOR_ORIENTATION,
                        (unsigned short)(base_rate / rate));
#endif
...
int handle_char_input(char c)
{
        ...
        case 'G':
                ...
                inv_enable_sensor(ANDROID_SENSOR_GYROSCOPE_UNCALIBRATED,
                        !!(hal.report & PRINT_RAW_GYRO));
                dmp_reset_odr_counters();
                if (hal.report & PRINT_RAW_GYRO)
                        set_output_rates(current_output_rate);
        ...
        case 'C':
                ...
                inv_enable_sensor(ANDROID_SENSOR_MAGNETIC_FIELD_UNCALIBRATED,
                        !!(hal.report & PRINT_RAW_COMPASS));
                dmp_reset_odr_counters();
                if (hal.report & PRINT_RAW_COMPASS)
                        set_output_rates(current_output_rate);
        ...
        case 'q':
                ...
                inv_enable_sensor(ANDROID_SENSOR_GAME_ROTATION_VECTOR,
                        !!(hal.report & PRINT_CUBE_GRV));
                dmp_reset_odr_counters();
                if (hal.report & PRINT_CUBE_GRV)
                        set_output_rates(current_output_rate);
        ...
        case 'd':
                ...
                inv_enable_sensor(ANDROID_SENSOR_STEP_DETECTOR,
                        !!(hal.report & PRINT_STEP_DETECTOR));
                dmp_reset_odr_counters();
#if (MEMS_CHIP == HW_ICM20609)
        ...
        case 'm':
                ...
                inv_enable_sensor(ANDROID_SENSOR_WAKEUP_SIGNIFICANT_MOTION,
                        !!(hal.report & PRINT_SMD));
                dmp_reset_odr_counters();
#if (MEMS_CHIP == HW_ICM20609)
        ...
        case 'e':
                ...
                inv_enable_sensor(ANDROID_SENSOR_GEOMAGNETIC_ROTATION_VECTOR,
                        !!(hal.report & PRINT_GEOMAG));
                dmp_reset_odr_counters();
                if (hal.report & PRINT_GEOMAG)
                        set_output_rates(current_output_rate);
        ...
        case 'b':
                ...
                inv_enable_sensor(ANDROID_SENSOR_ACTIVITY_CLASSIFICATON,
                        !!(hal.report & PRINT_BAC));
                dmp_reset_odr_counters();
                if (hal.report & PRINT_BAC)
                        set_output_rates(112.5);
        ...
        case 'l':
                ...
                inv_enable_sensor(ANDROID_SENSOR_LINEAR_ACCELERATION,
                        !!(hal.report & PRINT_LINEAR_ACCEL));
                dmp_reset_odr_counters();
                if (hal.report & PRINT_LINEAR_ACCEL)
                        set_output_rates(current_output_rate);
        ...
        case 't':
                dmp_reset_fifo();
                {
                        ...
                        INV_SPRINTF(tst, INV_TST_LEN, "\tX:%d Y:%d Z:%d\r\n", a_average[0],
                                a_average[1], a_average[2]);
                        print_command_console(tst);
                        INV_SPRINTF(tst, INV_TST_LEN, "Gyro Average (LSB@FSR 250dps)\r\n");
                        print_command_console(tst);
                        INV_SPRINTF(tst, INV_TST_LEN, "\tX:%d Y:%d Z:%d\r\n", g_average[0],
                                g_average[1], g_average[2]);
                        print_command_console(tst);
                        INV_SPRINTF(tst, INV_TST_LEN,
                                "Factory Cal - Accel DMP biases: \tX:%d Y:%d Z:%d\r\n", dmp_bias[0],
                                dmp_bias[1], dmp_bias[2]);
                        print_command_console(tst);
                        INV_SPRINTF(tst, INV_TST_LEN,
                                "Factory Cal - Gyro DMP biases:  \tX:%d Y:%d Z:%d\r\n", dmp_bias[3],
                                dmp_bias[4], dmp_bias[5]);
                        print_command_console(tst);
...
void process_sensor_output(unsigned short fifo_id)
{
        ...
        if (hal.report & PRINT_ACCEL) {
                if (accel_data_was_set == 1) {
                        ...
                        if (rble_data_patition_not_full)
                        {
                                ...
                                //rble_smp_count++;
                                memcpy(rble_sample_data, RBLE_DATA_ACCEL_LABLE,
                                        RBLE_DATA_LABLE_LENGTH);
                                rble_smp_count += RBLE_DATA_LABLE_LENGTH;

                                memcpy((rble_sample_data + rble_smp_count), &(accel_float[0]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
                                memcpy((rble_sample_data + rble_smp_count), &(accel_float[1]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
                                memcpy((rble_sample_data + rble_smp_count), &(accel_float[2]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;

                                if (rble_could_write_data_to_patition(rble_data_addr_offset,
                                        rble_smp_count))
                                        {
                                        ad_nvms_write(nvms_rble_storage_handle,
                                                rble_data_addr_offset, rble_sample_data,
                                                rble_smp_count);
        ...
        if (hal.report & PRINT_GYRO) {
                if (gyro_data_was_set == 1) {
                        ...
                        INV_SPRINTF(tst, INV_TST_LEN,
                                "Gyro Data\t %7.5f, %7.5f, %7.5f, %d, %lld\r\n", gyro_float[0],
                        ...
                        if (rble_data_patition_not_full)
                        {
                                ...
                                memcpy(rble_sample_data, RBLE_DATA_GYRO_LABLE,
                                        RBLE_DATA_LABLE_LENGTH);
                                rble_smp_count += RBLE_DATA_LABLE_LENGTH;

                                memcpy((rble_sample_data + rble_smp_count), &(gyro_float[0]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
                                memcpy((rble_sample_data + rble_smp_count), &(gyro_float[1]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
                                memcpy((rble_sample_data + rble_smp_count), &(gyro_float[2]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
        ...
        if (hal.report & PRINT_RAW_GYRO) {
                ...
                if (raw_gyro_data_was_set == 1) {
                        INV_SPRINTF(tst, INV_TST_LEN,
                                "Raw Gyro Data\t %7.5f, %7.5f, %7.5f,%d, %lld\r\n",
        ...
        if (quat6_data_was_set == 1) {
                if (hal.report & PRINT_GRV) {
                        INV_SPRINTF(tst, INV_TST_LEN,
                                "Game RV\t %7.5f, %7.5f, %7.5f, %7.5f, %lld\r\n", grv_float[0],
                                grv_float[1], grv_float[2], grv_float[3], ts);
                        print_data_console(tst);
                }
                if (hal.report & PRINT_CUBE_GRV) {
        ...
        if (hal.report & PRINT_COMPASS) {
                if (compass_data_was_set == 1) {
                        INV_SPRINTF(tst, INV_TST_LEN,
                                "Compass Data\t %7.5f, %7.5f, %7.5f,\t%d, %lld\r\n",
                        ...
                        if (rble_data_patition_not_full)
                        {
                                ...
                                memcpy(rble_sample_data, RBLE_DATA_COMPS_LABLE,
                                        RBLE_DATA_LABLE_LENGTH);
                                rble_smp_count += RBLE_DATA_LABLE_LENGTH;

                                memcpy((rble_sample_data + rble_smp_count), &(compass_float[0]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
                                memcpy((rble_sample_data + rble_smp_count), &(compass_float[1]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
                                memcpy((rble_sample_data + rble_smp_count), &(compass_float[2]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
        ...
        if (hal.report & PRINT_RAW_COMPASS) {
                if (raw_compass_data_was_set == 1) {
                        INV_SPRINTF(tst, INV_TST_LEN,
                                "Raw Compass Data\t %7.5f, %7.5f, %7.5f,\t%d, %lld\r\n",
                                compass_raw_float[0], compass_raw_float[1], compass_raw_float[2], 0,
                                ts);
                        print_command_console(tst);
        ...
        if (quat9_data_was_set == 1) {
                if (hal.report & PRINT_RV) {
                        INV_SPRINTF(tst, INV_TST_LEN,
                                "RV\t %7.5f, %7.5f, %7.5f, %7.5f, %d, %lld\r\n", rv_float[0],
                                rv_float[1], rv_float[2], rv_float[3], rv_accuracy, ts);
                        print_data_console(tst);
                }
                if (hal.report & PRINT_CUBE_RV) {
                ...
                if (hal.report & PRINT_ORIENT) {
                        ...
                        if (rble_data_patition_not_full)
                        {
                                ...
                                memcpy(rble_sample_data, RBLE_DATA_ORI_LABLE,
                                        RBLE_DATA_LABLE_LENGTH);
                                rble_smp_count += RBLE_DATA_LABLE_LENGTH;

                                memcpy((rble_sample_data + rble_smp_count), &(orientationFloat[0]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
                                memcpy((rble_sample_data + rble_smp_count), &(orientationFloat[1]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
                                memcpy((rble_sample_data + rble_smp_count), &(orientationFloat[2]),
                                        RBLE_FLOAT_SIZE);
                                rble_smp_count += RBLE_FLOAT_SIZE;
        ...
        if (gmrv_quat_was_set == 1) {
                ...
                {
                        INV_SPRINTF(tst, INV_TST_LEN,
                                "GeoMAG RV\t %7.5f, %7.5f, %7.5f, %7.5f, %d, %lld\r\n",
                                gmrv_float[0], gmrv_float[1], gmrv_float[2], gmrv_float[3],
                                gmrv_accuracy, ts);
                        print_data_console(tst);
                }
                if (hal.report & PRINT_CUBE_GEOMAG) {
	...
	if(rble_could_write_result_data_to_patition())
        ...
        fflush(stdout);
        memset(print_float_str,0,RBLE_FLOAT_STR_LENTH);
        qfp_float2str(acc_gyr_ori[1],print_float_str,0);
        printf("accy:");
        printf(print_float_str);
        printf("\n");
        fflush(stdout);
        memset(print_float_str,0,RBLE_FLOAT_STR_LENTH);
        qfp_float2str(acc_gyr_ori[2],print_float_str,0);
        printf("accz:");
        printf(print_float_str);
        printf("\n");
        fflush(stdout);
        memset(print_float_str,0,RBLE_FLOAT_STR_LENTH);
        qfp_float2str(acc_gyr_ori[3],print_float_str,0);
        printf("gyry:");
        printf(print_float_str);
        printf("\n");
        fflush(stdout);memset(print_float_str,0,RBLE_FLOAT_STR_LENTH);
        qfp_float2str(acc_gyr_ori[4],print_float_str,0);
        printf("yaw:");
        printf(print_float_str);
        printf("\n");
        fflush(stdout);
...
#if (MEMS_CHIP != HW_ICM20609)
        if (int_read_back
                & (BIT_MSG_DMP_INT | BIT_MSG_DMP_INT_0 | BIT_MSG_DMP_INT_2 | BIT_MSG_DMP_INT_5))
                #else
                ...
                do {
                        ...
                        if (inv_mems_fifo_swmirror(&data_left_in_fifo, &total_sample_cnt,
                                sample_cnt_array))
                                break;
...
#if defined(RBLE_DATA_STORAGE_IN_FLASH)
                        {
                                ...
                                if (rble_data_patition_not_full)
                                {
                                        ...
                                        memcpy(rble_sample_data, RBLE_DATA_TK_LABLE,
                                                RBLE_DATA_LABLE_LENGTH);
                                        rble_smp_count += RBLE_DATA_LABLE_LENGTH;

                                        memcpy((rble_sample_data + rble_smp_count), &tmp_tick,
                                                RBLE_LONG_SIZE);
                                        rble_smp_count += RBLE_LONG_SIZE;

                                        memcpy((rble_sample_data + rble_smp_count),
                                                RBLE_DATA_CNT_LABLE, RBLE_DATA_LABLE_LENGTH);
                                        rble_smp_count += RBLE_DATA_LABLE_LENGTH;

                                        memcpy((rble_sample_data + rble_smp_count),
                                                &total_sample_cnt, RBLE_CNT_SIZE);
                        ...
                        while (total_sample_cnt--) {
                                ...
                                if (header & GYRO_SET) {
                                        ...
                                        scale = (1 << inv_get_gyro_fullscale()) * 250.f
                                                / (1L << 15); // From raw to dps
                                        ...
                                        if (hal.report & PRINT_GYRO) {
                                                ...
                                                inv_mems_dmp_get_calibrated_gyro(long_data,
                                                        raw_data, bias_data);
                                                inv_convert_dmp3_to_body(long_data, scale,
                                                        gyro_float);
                                                gyro_data_was_set = 1;
                                        }
                                ...
                                if (header & QUAT6_SET) {
                                        ...
                                        inv_mems_augmented_sensors_get_gravity(gravityQ16,
                                                temp_gravityQ16);
                                        ...
                                        accelQ16[0] = (int32_t)((float)(accel_float[0])
                                                * (1ULL << 16) + ((accel_float[0] >= 0) - 0.5f));
                                        accelQ16[1] = (int32_t)((float)(accel_float[1])
                                                * (1ULL << 16) + ((accel_float[1] >= 0) - 0.5f));
                                        accelQ16[2] = (int32_t)((float)(accel_float[2])
                                                * (1ULL << 16) + ((accel_float[2] >= 0) - 0.5f));
                                        inv_mems_augmented_sensors_get_linearacceleration(linAccQ16,
                                                gravityQ16, accelQ16);
...
//test_r
                                ...
                                if (header & QUAT9_SET) {
                                        ...
                                        rv_accuracy = (int)((float)inv_get_rv_accuracy()
                                                / (float)(1ULL << (29)));
                                        ...
                                        if (hal.report & PRINT_ORIENT) {
...
#if 1
                                                inv_convert_rotation_vector_1(long_quat,
                                                        temp_orientationQ16);

                                                inv_mems_augmented_sensors_get_orientation(
                                                        orientationQ16, temp_orientationQ16);
#endif
                                                ...
                                                orientationFloat[0] = inv_q16_to_float(
                                                        orientationQ16[0]);
                                                orientationFloat[1] = inv_q16_to_float(
                                                        orientationQ16[1]);
                                                orientationFloat[2] = inv_q16_to_float(
                                                        orientationQ16[2]);
#endif
...
//test_r dump 	
                                ...
                                if (header & GEOMAG_SET) {
                                        ...
                                        if (hal.report & PRINT_GEOMAG) {
                                                ...
                                                gmrv_accuracy = (int)((float)inv_get_gmrv_accuracy()
                                                        / (float)(1ULL << (29)));
                                        }
                                        gmrv_quat_was_set = 1;
                                }
...
void rble_sample_create_timer(void)
{
        ...
        TickType_t period_tick = 51;	//102;         //51 (100ms);
                ...
                else if (notif & RBLE_SENSOR_STOP_SAMPLE_NOTIF)
                {
                        ...
                        if (OS_TIMER_IS_ACTIVE(rble_sample_timer_id))
                                ...
                                if ((RBLE_DATA_PATITION_SIZE - rble_data_addr_offset) > 3)
                                        ...
                                        ad_nvms_write(nvms_rble_storage_handle,
                                                rble_data_addr_offset, rble_storage_end,
