...
static void data_v2(FILE *file)
{
	...
	//for (i = 0; i < 1000; i++) {
		...
		detect_new_step_v5(samples_filter(giCsvData[i*giNumCol+1],0), samples_filter(giCsvData[i*giNumCol+2],1), (giCsvData[i*giNumCol+3]),i, giCsvData[i*giNumCol + 4],file);
...
{
	char *filename = "e:/python-project/test/num/ao_031605.txt";
	//char *filename = "e:/python-project/test/num/a_wbin0316.txt";
