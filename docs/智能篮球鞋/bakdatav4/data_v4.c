		...
		if (detect_peak(acc_z, step_env.acc_value_mode.acc_z_old)){
			...
			if (step_env.time_of_now - MAX(step_env.last_step.time, get_temp_step_time(JUMP)) > 110) {
				if (step_env.jump.flag == 0) {
					...
					if (((step_env.time_of_now - step_env.jump.fir_peak_time) > 120) && ((step_env.time_of_now - step_env.jump.fir_peak_time) < 450)) {
				...
				if (step_env.jump.flag == 2) {
					step_env.jump.value = (step_env.jump.fir_peak_value + step_env.jump.sec_peak_value) / 2;
