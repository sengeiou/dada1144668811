...
function downLoadNew(productId,fn){
	if(jQuery('.outerProg').length != 0){return false}
...
function addDocToBookshop(pid){
	...
	if(jQuery("#addBookShop2").length>0){jQuery("#addBookShop2").removeClass('green').addClass('gray').html('<span class="ico_mini ico_addfav"></span>已收藏');}
	jQuery.ajax({
		type:'GET',
			...
			if(data){
				...
				if(index == 0 || index2 == 0){//文档可直接下载
					...
					if (typeof history.pushState == "function") {
						...
						var name=downPath.substring(downPath.indexOf("filename=")+9,downPath.length);
						var name=decodeURI(decodeURI(name));
						download(downPath, function(blob){saveAs(blob,name);},pid);
						return;
					}else{
						downloadSuccess(pid);
						jQuery('body').append('<iframe style="display:none" id="iframe_data" src="'+downPath+'"></iframe>');
				...
				//if(data.indexOf("downLoadTips10") > -1 ){//下载验证码
					//var oHtml = data;
					//jQuery(this).append(oHtml);
					//return;
				//}
...
function clickPayRead(pid,uid){
	//先判断是否登陆
	if(isUserLogin == 0){
		showPayReadLogin("payReadValue","&registerFrom=1");
	}else{
		//加载付费阅读 弹窗
